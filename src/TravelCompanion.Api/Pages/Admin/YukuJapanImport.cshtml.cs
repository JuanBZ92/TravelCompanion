...
public sealed class YukuJapanImportModel(
    YukuJapanRecommendationImportService importService) : PageModel
{
    private const int MaxWorkbookBytes = 2 * 1024 * 1024;
    ...
    public YukuJapanRecommendationImportResult? ImportResult { get; private set; }
    ...
    private async Task<byte[]?> ReadWorkbookBytesAsync(bool allowPreviewPayload = false)
    {
        if (Workbook is not null && Workbook.Length > 0)
        {
            ...
            {
                ModelState.AddModelError(nameof(Workbook), "El archivo no puede superar 2 MB.");
                return null;
            }
        ...
        ModelState.AddModelError(nameof(Workbook), "Subi el Excel YUKU Japan verificado.");
        return null;
    }
}
